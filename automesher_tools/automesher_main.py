...
from decorate_oroginal_methods import CSX_Copy, FDTD_copy
import geometry_utils, meshing_utils, mesh_processing
...
class Automesher:
    ...
    def get_mesh_data(self, polygon, **kw):
                    ...
                    if primitives:
                        if isinstance(primitives, list):
                            ...
                            for prim in primitives:
                                geometry_utils.process_primitive(prim, tmp_x, tmp_y, tmp_x_edges, tmp_y_edges, tmp_diagonal_edges)
                            ...
                            geometry_utils.process_single_polygon(primitives, tmp_x, tmp_y, tmp_x_edges, tmp_y_edges, tmp_diagonal_edges)
        ...
        meshing_utils.get_mesh_parameters(self)
        ...
        meshing_utils.handle_otheredges(self, diagonal_edges, unique_xedges, unique_yedges, self.mesh_res, self.max_res, mesh_data[1], 'y')

        # Refine mesh in small gaps for x and y edges
        meshing_utils.mesh_small_gaps(self, x_edges, self.mesh_res, self.max_res, self.num_lines, mesh_data, 'x')
        meshing_utils.mesh_small_gaps(self, y_edges, self.mesh_res, self.max_res, self.num_lines, mesh_data, 'y')
        meshing_utils.mesh_small_gaps(self, z_coords, self.mesh_res, self.max_res, self.num_lines, mesh_data, 'z')
        ...
        meshing_utils.add_missing_mesh_lines(self, unique_xedges, sorted_x, diagonal_edges, self.mesh_res, mesh_data[0], 'x')
        meshing_utils.add_missing_mesh_lines(self, unique_yedges, sorted_y, diagonal_edges, self.mesh_res, mesh_data[1], 'y')
        ...
        meshing_utils.handle_circular_segments(self, polygon, mesh_data)
        ...
        mesh_data[0] = meshing_utils.add_ports_to_mesh_data(self, mesh_data[0], x_edges, 'x')
        mesh_data[1] = meshing_utils.add_ports_to_mesh_data(self, mesh_data[1], y_edges, 'y')
        mesh_data[2] = meshing_utils.add_ports_to_mesh_data(self, mesh_data[2], z_coords, 'z')

        # Smooth and process the mesh lines
        mesh_processing.smooth_and_process_mesh_lines(self, mesh_data, polygon, x_edges, y_edges, z_coords, unique_xedges, unique_yedges, z_coords, mesh_map) 

        # Chech if all edges are in the mesh_data
        meshing_utils.add_edges_to_mesh_mesh_data(self, mesh_data[0], x_edges, self.mesh_res, self.min_cellsize, 'x')
        meshing_utils.add_edges_to_mesh_mesh_data(self, mesh_data[1], y_edges, self.mesh_res, self.min_cellsize, 'y')
        meshing_utils.add_edges_to_mesh_mesh_data(self, mesh_data[2], z_coords, self.mesh_res, self.min_cellsize, 'z')

        arcs = geometry_utils.detect_all_arcs_in_polygon(self, polygon)
